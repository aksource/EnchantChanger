...
public class Face
{
    ...
    public void addFaceForRender(Tessellator tessellator, float textureOffset)
    {
        ...
        buffer.putNormal(faceNormal.x, faceNormal.y, faceNormal.z);
        ...
        for (int i = 0; i < vertices.length; ++i)
        {
            ...
            if ((textureCoordinates != null) && (textureCoordinates.length > 0))
            {
                ...
                buffer.pos(vertices[i].x, vertices[i].y, vertices[i].z).tex(textureCoordinates[i].u + offsetU, textureCoordinates[i].v + offsetV).endVertex();
            }
            else
            {
                buffer.pos(vertices[i].x, vertices[i].y, vertices[i].z).endVertex();
            }
        }
    }
