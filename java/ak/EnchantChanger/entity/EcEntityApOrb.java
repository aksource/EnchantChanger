...
//		this.yOffset = this.height / 2.0F;
    ...
    public void onUpdate() {
        ...
        if (this.worldObj.getBlockState(new BlockPos(this.posX, this.posY, this.posZ)).getBlock().getMaterial() == Material.lava) {
        ...
        if (this.onGround) {
            var14 = this.worldObj.getBlockState(new BlockPos(this.posX, this.getEntityBoundingBox().minY - 1, this.posZ)).getBlock().slipperiness * 0.98F;
        }
...
//		}
    ...
    private boolean checkLevelLimit(Enchantment ench, int nowLevel) {
        if (ench == null) {
            return true;
        } else if (EnchantmentUtils.LEVEL_LIMIT_MAP.containsKey(ench.effectId)) {
            if (EnchantmentUtils.LEVEL_LIMIT_MAP.get(ench.effectId) == 0) {
                return ench.getMaxLevel() <= nowLevel;
            } else {
                return EnchantmentUtils.LEVEL_LIMIT_MAP.get(ench.effectId) <= nowLevel;
            }
        } else
