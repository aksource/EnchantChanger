...
public class RenderingOverlayEvent {
    ...
    public void overlayEvent(RenderGameOverlayEvent.Text event) {
        ...
        if (holdItem != null && holdItem.getItem() instanceof EcItemSword) {
            ...
            renderLimitGauge(holdItem, slot, width, height, event.partialTicks);
    ...
    private void renderLimitGauge(ItemStack holdItem, int slot, int width, int height, float partialTicks) {
        int limitGaugeValue = EcItemSword.getLimitGaugeValue(holdItem);
        int limitGaugeMaxValue = EcItemSword.LIMIT_GAUGE_MAX;
...
//        renderCustomStackSizeLikeString(valueString, x, z);
    ...
    private void renderCustomDurationLikeBar(double value, int x, int y) {
        ...
        this.renderQuad(tessellator, x + 2, y + 13, 13, 2, 0xFFFFFF);
